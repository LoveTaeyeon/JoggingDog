...
import edu.fjnu.DAOImpl.HealthPlanDAOImpl;
import edu.fjnu.Entity.Plan.HealthPlan;
import edu.fjnu.Service.Translater.Translater;

public class GetPlanForPeople {

	public static void main(String[] args) {
		List<HealthPlan> hea = GetPlanForPeople.getPlan("无常_", "引体向上", "良好");
		for(int i = 0;i < hea.size();i ++){
			System.out.println(hea.get(i).toString());
		}
	}
	
	public static List<HealthPlan> getPlan(String userID,String forWhat,String level){		//算法，向用户推荐方案的算法
...
//			List<HealthPlan> suitForEveryOne = new ArrayList<HealthPlan>();		//针对核心肌群
//			List<HealthPlan> suitForThisPlay = new ArrayList<HealthPlan>();		//针对单一运动
//			try {				//取出核心肌群和该运动里面适合各个等级的，准备加入推荐方案中，增加容错率
//				suitForEveryOne = hea.queryByForwhatAndLevel("核心肌群",level);
//				//suitForThisPlay = hea.queryByForwhatAndLevel(forWhat, "适合各个等级");
//			} catch (SQLException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			}
			...
			/*根据体脂率决定运动强度等级的分配
			 * 正常状态下，各种强度应该均匀分配
			 * 偏瘦状态下，少量剧烈运动，中低强度运动偏多
			 * 超重状态下，应该中等强度运动居多
			 * 肥胖状态下，不宜进行剧烈运动，应以低强度运动居多
			 * */
