...
//			 if (datadate.equals("2015-12-08")){
	...
	private static CodoonData getCoreCodoonData(String dataStr) {
		...
		for (Element element2 : todaydataElements) {
			...
			if (i == 1 || i == 5 || i == 9) {
				// 跳过运动时长
				String[] tempStr = element2.text().split(":");
				System.out.println("tempStr:"+tempStr[0]+" "+tempStr[1]);
				tempdata[i] = Integer.valueOf(tempStr[0])*60+Integer.valueOf(tempStr[1]);
				i++;
			} else {
		...
		todayCodoonData
				.setDuration((int) (tempdata[1] + todayCodoonData.getDuration()));
